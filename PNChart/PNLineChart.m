...
- (void)setYLabels {
    CGFloat yStep = (_yValueMax - _yValueMin) / _yLabelLeftNum;
    CGFloat yStepHeight = _chartCavanHeight / _yLabelLeftNum;

    if (_yChartLeftLabels) {
        for (PNChartLabel *label in _yChartLeftLabels) {
            [label removeFromSuperview];
        }
    } else {
        _yChartLeftLabels = [NSMutableArray new];
    }
    ...
    if (yStep == 0.0) {
        ...
        [_yChartLeftLabels addObject:minLabel];
        ...
        [_yChartLeftLabels addObject:midLabel];
        ...
        [_yChartLeftLabels addObject:maxLabel];

    } else {
        NSInteger index = 0;
        NSInteger num = _yLabelLeftNum + 1;
        ...
        while (num > 0) {
            ...
            [_yChartLeftLabels addObject:label];
            index += 1;
            num -= 1;
        }
    }
...
-(void)setYLeftLabels:(NSArray *)yLeftLabels
{
    _showGenYLabels = NO;
    _yLabelLeftNum = yLeftLabels.count - 1;

    CGFloat yLabelHeight;
    if (_showLabel) {
        yLabelHeight = _chartCavanHeight / [yLeftLabels count];
    } else {
        yLabelHeight = (self.frame.size.height) / [yLeftLabels count];
    }

    return [self setyLeftLabels:yLeftLabels withHeight:yLabelHeight];
}

- (void)setyLeftLabels:(NSArray *)yLeftLabels withHeight:(CGFloat)height {
    _yLeftLabels = yLeftLabels;
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLeftLabels.count; index++) {
            labelText = yLeftLabels[(NSUInteger) index];
...
{
    _showGenYLabels = NO;
    _yLabelRightNum = yRightLabels.count - 1;
    
    CGFloat yLabelHeight;
    if (_showLabel) {
        yLabelHeight = _chartCavanHeight / [yRightLabels count];
    } else {
        yLabelHeight = (self.frame.size.height) / [yRightLabels count];
    }
    
    return [self setyRightLabels:yRightLabels withHeight:yLabelHeight];

}

//回执
- (void)setyRightLabels:(NSArray *)yLeftLabels withHeight:(CGFloat)height {
    _yLeftLabels = yLeftLabels;
    _yLabelHeight = height;
    if (_yChartRightLabels) {
        for (PNChartLabel *label in _yChartLeftLabels) {
            [label removeFromSuperview];
        }
    } else {
        _yChartRightLabels = [NSMutableArray new];
    }
    
    NSString *labelText;
    
    if (_showLabel) {
        CGFloat yStepHeight = _chartCavanHeight / _yLabelRightNum;
        ...
        for (int index = 0; index < yLeftLabels.count; index++) {
            ...
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(_chartCavanWidth+10, y, (CGFloat) ((NSInteger) _chartMarginLeft * 0.9), (NSInteger) _yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
            label.text = labelText;
            [self setCustomStyleForYLabel:label];
            [self addSubview:label];
            [_yChartRightLabels addObject:label];
        }
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.showYGridLines) {
        ...
        for (NSUInteger i = 0; i < _yLabelLeftNum; i++) {
